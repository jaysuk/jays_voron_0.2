...
while state.time < var.RunTime
	...
	if (var.RunTime - state.time) > 600
		M291 R"Soak Time" T{var.Delay} S1 P{var.HoursLeft ^ " hrs : " ^ var.MinutesLeft ^ " mins"}
		G4 S{var.Delay}
	elif (var.RunTime - state.time) > 60 
		set var.Delay = var.ShortDelay
		M291 R"Soak Time" T{var.Delay} S1 P{var.HoursLeft ^ " hrs : " ^ var.MinutesLeft ^ " mins : " ^ var.SecondsLeft ^ " secs"}
		G4 S{var.Delay}
	elif (var.RunTime - state.time) > 10 
		set var.Delay = 5
		M291 R"Print Starting in..." T{var.Delay} S1 P{floor(var.RunTime - state.time) ^ " seconds"}
		M300 S{var.BeepFrequencyLow} P{var.BeepDuration}
		G4 S{var.Delay}
	else
		set var.Delay = 1
		M291 R"Print Starting in..." T{var.Delay} S0 P{floor(var.RunTime - state.time) ^ " seconds"}
		M300 S{var.BeepFrequencyHigh} P{var.BeepDuration}
		G4 S{var.Delay}
	if exists(param.X) && (var.Loops = param.X)
		break
M118 S"Starting Print after Soak Time" 
